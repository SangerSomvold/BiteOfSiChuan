...
public class SearchResultActivity extends Activity implements OnClickListener {
	...
	mPwClassify = new SearchResultPopClassifyWindow(
		SearchResultActivity.this, myAdapter, mClassify);
	...
	types.getTypeList(new IOnHttpRequeseListener() {
		...
		mClassify = new String[alTd.size()];
		for (int i = 0; i < mClassify.length; i++) {
		    mClassify[i] = alTd.get(i).typeName;
		}
		...
		case 1:
		    mTvSearchKeyword.setText("搜索\""
			    + mClassify[myAdapter.mSearchType - 1] + "\"");
		    break;
		}
